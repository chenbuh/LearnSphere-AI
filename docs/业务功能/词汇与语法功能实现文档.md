...
### 4.1 词汇界面 (`VocabularyView.vue` & `LearningHubView.vue`)
*   **学习中心动态闪卡**: `LearningHubView` 现在通过 `vocabularyApi.getDailyWords` 动态获取每日单词，而非固定内容。系统会根据用户的 `examType` 推荐合适的词汇。
*   **抽认卡模式 (Flashcard)**: 正面单词，背面释义与例句。支持键盘快捷键（认识/不认识）。
*   **拼写测试**: 听音拼写模式。

### 4.2 听力界面 (`ListeningView.vue` & `LearningHubView.vue`)
*   **学习中心每日练习**: `LearningHubView` 现在会从用户的听力历史中提取最新的练习内容进行展示。如果暂无历史记录，将展示基于当前日期的推荐内容。
