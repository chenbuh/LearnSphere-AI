...
public class AIGenerationServiceImpl implements IAIGenerationService {
    ...
    public Map<String, Object> generateListening(String type, String difficulty, Integer count) {
        ...
        try {
            ...
            String userPrompt = String.format(
                    "请生成%d段%s难度的英语听力对话/短文，每段包含标题(title)、对话内容(audioScript)和5道选择题。\n" +
                            "【重要】必须严格生成 %d 段内容，不能少。\n" +
                            "不要使用 Markdown 格式，不要包含任何解释性文字，直接返回纯 JSON 字符串。\n" +
                            "返回JSON：{\"passages\":[{\"title\":\"短文标题\", \"audioScript\":\"对话内容\", \"questions\":[{\"text\":\"问题\", \"options\":[\"A\",\"B\",\"C\",\"D\"], \"correct\":0, \"explanation\":\"解析\"}]}]}\n",
                        ...
                        for (Object pObj : (Iterable<?>) passagesObj) {
                            if (pObj instanceof Map) {
                                ...
                                if (aiTitle != null && !aiTitle.isEmpty()) {
                                    ...
                                    lm.setTitle(type + " Practice - " + difficulty);
                                }
            ...
            if (passagesObj instanceof Iterable) {
                for (Object pObj : (Iterable<?>) passagesObj) {
                    if (pObj instanceof Map) {
                        ...
                        lm.setTitle(title != null && !title.isEmpty() ? title : (type + " Practice - " + difficulty));
                        lm.setScript((String) p.get("audioScript"));
                        Object q = p.get("questions");
    ...
    private Map<String, Object> generateMockListening(String difficulty, Integer count) {
        ...
        for (int i = 0; i < actualCount; i++) {
            passages.add(Map.of(
                    "audioScript",
                    "Passage " + (i + 1)
                            + ": This is a mock listening script for testing purposes. It simulates a conversation or monologue relevant to the "
                            + difficulty + " difficulty level.",
                    "questions", List.of(
                            Map.of(
                                    "text", "What is the main topic of Passage " + (i + 1) + "?",
                                    "options", List.of("Topic A", "Topic B", "Topic C", "Topic D"),
                                    "correct", 0,
                                    "explanation", "This is a mock explanation.")),
                    "title", "Mock Passage " + (i + 1)));
        }
        ...
        try {
            ...
            if (overall != null) {
                statsSummary.append(String.format("总练习数: %s, 正确率: %s%%. ",
                        overall.get("total_records"),
                        (int) (((Number) overall.get("accuracy")).doubleValue() * 100)));
            }
