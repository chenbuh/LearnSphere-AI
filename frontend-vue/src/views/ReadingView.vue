...
const fetchHistory = async () => {
    try {
        const res = await aiApi.getReadingHistory()
        if (res.code === 200) {
...
const wrongAnswers = computed(() => {
    ...
    return article.value.questions.filter((q, idx) => answers.value[idx] !== q.correct)
})
</script>
    ...
    <div v-if="step === 'setup'" class="setup-container">
       ...
       <div v-if="historyArticles.length > 0" class="history-section">
            ...
            <n-grid x-gap="16" y-gap="16" cols="1 600:2 900:3">
                <n-grid-item v-for="item in historyArticles" :key="item.id">
    ...
    <div v-else-if="step === 'reading'" class="reading-container" v-if="article">
       ...
       <div class="progress-bar-container">
            <div class="progress-info">
                <span>阅读进度</span>
                <span>{{ currentQuestionIndex + 1 }} / {{ article.questions.length }}</span>
            </div>
            <n-progress 
                type="line" 
