...
import { 
    ...
    NStatistic, NNumberAnimation, NSpace, NDivider
} from 'naive-ui'
import { Search, Volume2, Trophy, Brain, Check, X, BookOpen, RotateCw, Layers, Zap } from 'lucide-vue-next'
...
const startPlayback = (text) => {
  ...
  if (isSentence) {
      ...
      audio.onplay = () => console.log('[Audio] ✓ Sentence playing')
      audio.onerror = () => {
          console.log('[Audio] Trying backup TTS...')
          // 备选：使用系统TTS
      ...
      const audio = new Audio(url)
      currentAudio.value = audio
      audio.onplay = () => console.log('[Audio] ✓ Word playing')
...
.meaning-text { font-size: 2rem; font-weight: 700; color: #fff; margin-bottom: 32px; text-align: center; }
.example-box {
