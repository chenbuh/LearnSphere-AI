...
// 确保词汇数据库正确初始化
function initializeVocabularyDatabase() {
    try {
        if (!window.vocabularyDatabase) {
            console.log('🔄 正在初始化词汇数据库...');
            const vocabularyDatabase = new VocabularyDatabase();
            window.VocabularyDatabase = VocabularyDatabase;
            window.vocabularyDatabase = vocabularyDatabase;
            console.log('✅ 词汇数据库已成功加载，共', vocabularyDatabase.getTotalWordCount(), '个单词');
            
            // 触发数据库加载完成事件
            if (typeof window !== 'undefined' && window.document) {
                window.document.dispatchEvent(new CustomEvent('vocabularyDatabaseLoaded', {
                    detail: { database: vocabularyDatabase }
                }));
            }
        }
        return true;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(initializeVocabularyDatabase, 100);
    });
} else {
    // DOM已加载完成，延迟初始化确保其他脚本已加载
    setTimeout(initializeVocabularyDatabase, 100);
}
...
window.addEventListener('load', () => {
    setTimeout(() => {
        if (!window.vocabularyDatabase) {
            console.warn('⚠️ 词汇数据库仍未加载，尝试最后一次初始化');
            initializeVocabularyDatabase();
        }
    }, 500);
});
...
if (typeof module !== 'undefined' && module.exports) {
    ...
    // 确保全局变量可用
    window.VocabularyDatabase = VocabularyDatabase;
    window.initializeVocabularyDatabase = initializeVocabularyDatabase; // 暴露初始化函数
    if (!window.vocabularyDatabase) {
        initializeVocabularyDatabase();
    }
}
